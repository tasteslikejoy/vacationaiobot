...
class Pag(CallbackData, prefix='pag'): # это префикс, который будет использоваться для коллбек-данных, чтобы их было легче идентифицировать при обработке
...
def pag(page: int=0):
    ...
    builder.row(
        InlineKeyboardButton(text='⬅', callback_data=Pag(action='prev', page=page).pack()), # создается коллбек-данные для кнопки. Метод pack() упаковывает данные в строку, которую можно передать при нажатии на кнопку
